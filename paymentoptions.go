...
func newPaymentOptions(rootSDK *Gr4vy, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *PaymentOptions {
...
func (s *PaymentOptions) List(ctx context.Context, paymentOptionRequest components.PaymentOptionRequest, merchantAccountID *string, opts ...operations.Option) (*components.PaymentOptions, error) {
	...
	opURL, err := url.JoinPath(baseURL, "/payment-options")
	if err != nil {
	...
	bodyReader, reqContentType, err := utils.SerializeRequestBody(ctx, request, false, false, "PaymentOptionRequest", "json", `request:"mediaType=application/json"`)
	if err != nil {
		return nil, err
	}
