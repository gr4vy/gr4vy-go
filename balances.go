...
func newBalances(rootSDK *Gr4vy, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *Balances {
	return &Balances{
		rootSDK:          rootSDK,
...
func (s *Balances) List(ctx context.Context, giftCardBalanceRequest components.GiftCardBalanceRequest, merchantAccountID *string, opts ...operations.Option) (*components.GiftCardSummaries, error) {
	...
	opURL, err := url.JoinPath(baseURL, "/gift-cards/balances")
	if err != nil {
	...
	bodyReader, reqContentType, err := utils.SerializeRequestBody(ctx, request, false, false, "GiftCardBalanceRequest", "json", `request:"mediaType=application/json"`)
	if err != nil {
		return nil, err
	}
