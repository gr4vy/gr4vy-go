...
func newTransactions(rootSDK *Gr4vy, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *Transactions {
	return &Transactions{
		...
		RefundSettlements: newRefundSettlements(rootSDK, sdkConfig, hooks),
...
func (s *Transactions) Cancel(ctx context.Context, transactionID string, merchantAccountID *string, opts ...operations.Option) (*components.TransactionCancel, error) {
	request := operations.CancelTransactionRequest{
		TransactionID:     transactionID,
		MerchantAccountID: merchantAccountID,
	}

	globals := operations.CancelTransactionGlobals{
		MerchantAccountID: s.sdkConfiguration.Globals.MerchantAccountID,
	}
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/transactions/{transaction_id}/cancel", request, globals)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out components.TransactionCancel
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}
...
// IncrementAuthorization - Increment transaction authorization
// Increment the transaction authorization amount of a given transaction_id.
func (s *Transactions) IncrementAuthorization(ctx context.Context, transactionID string, transactionAuthorizationIncrementCreate components.TransactionAuthorizationIncrementCreate, merchantAccountID *string, idempotencyKey *string, opts ...operations.Option) (*components.TransactionAuthorizationIncrement, error) {
	request := operations.IncrementTransactionAuthorizationRequest{
		TransactionID:                           transactionID,
		MerchantAccountID:                       merchantAccountID,
		IdempotencyKey:                          idempotencyKey,
	...
	globals := operations.IncrementTransactionAuthorizationGlobals{
		MerchantAccountID: s.sdkConfiguration.Globals.MerchantAccountID,
	}
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/transactions/{transaction_id}/authorization/increment", request, globals)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	bodyReader, reqContentType, err := utils.SerializeRequestBody(ctx, request, false, false, "TransactionAuthorizationIncrementCreate", "json", `request:"mediaType=application/json"`)
	if err != nil {
		return nil, err
	}
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out components.TransactionAuthorizationIncrement
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}
