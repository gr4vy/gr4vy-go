...
func (a *TransactionsApiService) CaptureTransactionExecute(r ApiCaptureTransactionRequest) (Transaction, *_nethttp.Response, error) {
	...
	if localVarHTTPResponse.StatusCode >= 300 {
		...
		if localVarHTTPResponse.StatusCode == 404 {
			var v OneOfError404NotFoundError404PendingCreation
			err = a.client.decode(&v, localVarBody, localVarHTTPResponse.Header.Get("Content-Type"))
			if err != nil {
				newErr.error = err.Error()
				return localVarReturnValue, localVarHTTPResponse, newErr
...
// Filters the transactions to only the items for which the &#x60;id&#x60; or &#x60;external_identifier&#x60; matches this value. This field allows for a partial match, matching any transaction for which either of the fields partially or completely matches.
func (r ApiListTransactionsRequest) Search(search string) ApiListTransactionsRequest {
	r.search = &search
	return r
}
// Filters the results to only the transactions for which the &#x60;status&#x60; matches this value.
...
// Filters the results to only transactions last updated before this ISO date-time string.
...
// Filters the results to only transactions last updated after this ISO date-time string.
