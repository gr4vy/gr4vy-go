/*
 * Gr4vy API
 *
 * Welcome to the Gr4vy API reference documentation. Our API is still very much a work in product and subject to change.
 *
 * API version: 1.1.0-beta
 * Contact: code@gr4vy.com
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package Openapi

import (
	"encoding/json"
)

// GiftCardServiceUpdateRequestFields A key-value pair that represents a field defined in the definition for this gift card service.
type GiftCardServiceUpdateRequestFields struct {
	// The key of the field to set a value for.
	Key string `json:"key"`
	// The value of a field to set; you can unset optional fields by setting this to an empty string `\"\"`.
	Value string `json:"value"`
}

// NewGiftCardServiceUpdateRequestFields instantiates a new GiftCardServiceUpdateRequestFields object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGiftCardServiceUpdateRequestFields(key string, value string) *GiftCardServiceUpdateRequestFields {
	this := GiftCardServiceUpdateRequestFields{}
	this.Key = key
	this.Value = value
	return &this
}

// NewGiftCardServiceUpdateRequestFieldsWithDefaults instantiates a new GiftCardServiceUpdateRequestFields object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGiftCardServiceUpdateRequestFieldsWithDefaults() *GiftCardServiceUpdateRequestFields {
	this := GiftCardServiceUpdateRequestFields{}
	return &this
}

// GetKey returns the Key field value
func (o *GiftCardServiceUpdateRequestFields) GetKey() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Key
}

// GetKeyOk returns a tuple with the Key field value
// and a boolean to check if the value has been set.
func (o *GiftCardServiceUpdateRequestFields) GetKeyOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Key, true
}

// SetKey sets field value
func (o *GiftCardServiceUpdateRequestFields) SetKey(v string) {
	o.Key = v
}

// GetValue returns the Value field value
func (o *GiftCardServiceUpdateRequestFields) GetValue() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Value
}

// GetValueOk returns a tuple with the Value field value
// and a boolean to check if the value has been set.
func (o *GiftCardServiceUpdateRequestFields) GetValueOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Value, true
}

// SetValue sets field value
func (o *GiftCardServiceUpdateRequestFields) SetValue(v string) {
	o.Value = v
}

func (o GiftCardServiceUpdateRequestFields) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["key"] = o.Key
	}
	if true {
		toSerialize["value"] = o.Value
	}
	return json.Marshal(toSerialize)
}

type NullableGiftCardServiceUpdateRequestFields struct {
	value *GiftCardServiceUpdateRequestFields
	isSet bool
}

func (v NullableGiftCardServiceUpdateRequestFields) Get() *GiftCardServiceUpdateRequestFields {
	return v.value
}

func (v *NullableGiftCardServiceUpdateRequestFields) Set(val *GiftCardServiceUpdateRequestFields) {
	v.value = val
	v.isSet = true
}

func (v NullableGiftCardServiceUpdateRequestFields) IsSet() bool {
	return v.isSet
}

func (v *NullableGiftCardServiceUpdateRequestFields) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGiftCardServiceUpdateRequestFields(val *GiftCardServiceUpdateRequestFields) *NullableGiftCardServiceUpdateRequestFields {
	return &NullableGiftCardServiceUpdateRequestFields{value: val, isSet: true}
}

func (v NullableGiftCardServiceUpdateRequestFields) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGiftCardServiceUpdateRequestFields) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


