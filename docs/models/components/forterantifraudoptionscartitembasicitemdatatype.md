# ForterAntiFraudOptionsCartItemBasicItemDataType

## Example Usage

```go
import (
	"github.com/gr4vy/gr4vy-go/models/components"
)

value := components.ForterAntiFraudOptionsCartItemBasicItemDataTypeTangible

// Open enum: custom values can be created with a direct type cast
custom := components.ForterAntiFraudOptionsCartItemBasicItemDataType("custom_value")
```


## Values

| Name                                                         | Value                                                        |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| `ForterAntiFraudOptionsCartItemBasicItemDataTypeTangible`    | TANGIBLE                                                     |
| `ForterAntiFraudOptionsCartItemBasicItemDataTypeNonTangible` | NON_TANGIBLE                                                 |