...
```go
gr4vygo.WithServerURL("https://api.example.com")
```
...
```go
gr4vygo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
gr4vygo.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
gr4vygo.WithTimeout(30 * time.Second)
```
